          ...
          <div key={service.id} className="glass-panel rounded-2xl overflow-hidden flex flex-col hover:transform hover:-translate-y-1 transition-all duration-300 group">
            ...
            <div className="p-6 flex-grow flex flex-col">
              ...
              <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-200/50">
                <span className="text-2xl font-bold text-blue-700">R$ {service.price.toFixed(2)}</span>
                <button onClick={() => handleAdd(service)} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm transition-all shadow-md ${addedIds.includes(service.id) ? 'bg-green-600 text-white shadow-green-500/30' : 'bg-slate-900 text-white hover:bg-slate-800 shadow-slate-900/30'}`}>
                  {addedIds.includes(service.id) ? (<><Check className="h-4 w-4" /> Adicionado</>) : (<><Plus className="h-4 w-4" /> Adicionar</>)}
                </button>
              </div>
            </div>
          </div>
