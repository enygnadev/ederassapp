...
// Item in the shopping cart (Files are local objects)
export interface CartItem extends ServiceProduct {
  cartId: string;
  uploadedDocs: Record<string, File | null>;
}

// Item in a processed Order (Files are Strings/URLs)
export interface OrderItem extends ServiceProduct {
  cartId: string;
  uploadedDocs: Record<string, string>; // URL to Firebase Storage
}
